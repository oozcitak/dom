...
$$.suite('TreeWalker', () => {
  ...
  $$.test('previousNode() with type filter', () => {
    ...
    $$.deepEqual(str, ':child2:child1:node1')
  })
  ...
  $$.test('previousNode() with user filter', () => {
    ...
    $$.deepEqual(str, ':child1:node1')
  })

  $$.test('previousNode() null check', () => {
    const iter = doc.createTreeWalker(node1, $$.WhatToShow.ProcessingInstruction)
    iter.nextNode()
    $$.deepEqual(iter.previousNode(), null)
  })

  $$.test('currentNode getter', () => {
    const iter = doc.createTreeWalker(node1, $$.WhatToShow.Element)
    $$.deepEqual(iter.currentNode, node1)
  })
  ...
  $$.test('currentNode setter', () => {
    ...
    iter.nextNode()
    $$.deepEqual(iter.currentNode, child2)
  })

  $$.test('parentNode()', () => {
    const iter = doc.createTreeWalker(node1, $$.WhatToShow.Element)
    iter.nextNode()
    $$.deepEqual(iter.parentNode(), node1)
  })

  $$.test('parentNode() null check', () => {
    const iter = doc.createTreeWalker(node1, $$.WhatToShow.Element)
    $$.deepEqual(iter.parentNode(), null)
  })
  ...
  $$.test('firstChild()', () => {
    ...
    if (!child1)
      throw new Error("firstElementChild is null")
    $$.deepEqual(iter.firstChild(), child1)
  })
  ...
  $$.test('lastChild()', () => {
    ...
    $$.deepEqual(iter.lastChild(), child2)
  })
  ...
  $$.test('nextSibling()', () => {
    ...
    iter.firstChild()
    $$.deepEqual(iter.nextSibling(), child2)
  })
  ...
  $$.test('previousSibling()', () => {
    ...
    iter.lastChild()
    $$.deepEqual(iter.previousSibling(), child1)
  })
