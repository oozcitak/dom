...
export class XMLSerializerImpl implements XMLSerializer {
  ...
  private _xmlSerialization(node: Node, requireWellFormed: boolean): string {
    ...
    if (node._nodeDocument === undefined || node._nodeDocument._hasNamespaces) {
       ...
       */
      try {
        return this._serializeNodeNS(node, namespace, prefixMap, prefixIndex,
          requireWellFormed)
      } catch {
  ...
  private _serializeNodeNS(node: Node, namespace: string | null,
    ...
    switch (node.nodeType) {
      case NodeType.Element:
        return this._serializeElementNS(<Element>node, namespace, prefixMap, prefixIndex,
          requireWellFormed)
      case NodeType.Document:
        return this._serializeDocumentNS(<Document>node, namespace, prefixMap,
      ...
      case NodeType.DocumentFragment:
        return this._serializeDocumentFragmentNS(<DocumentFragment>node, namespace,
  ...
  private _serializeElementNS(node: Element, namespace: string | null,
     ...
     */
    markup += this._serializeAttributesNS(node, map, prefixIndex, localPrefixesMap,
  ...
  private _serializeDocumentNS(node: Document, namespace: string | null,
    ...
    */
    ...
    for (const childNode of node._children || node.childNodes) {
      serializedDocument += this._serializeNodeNS(childNode, namespace, prefixMap,
        prefixIndex, requireWellFormed)
    }
  ...
  private _serializeDocumentFragmentNS(node: DocumentFragment,
    ...
    let markup = ""
    for (const childNode of node._children || node.childNodes) {
      markup += this._serializeNodeNS(childNode, namespace, prefixMap,
        prefixIndex, requireWellFormed)
    }
    return markup
  }
