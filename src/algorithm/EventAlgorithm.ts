 ...
 */
export function event_initialize(event: Event, type: string, bubbles: boolean, cancelable: boolean): void {
 ...
 * @param realm - realm
 */
export function event_createAnEvent(eventInterface: typeof EventImpl, realm: any | undefined = undefined): Event {
  /**
  ...
  const dictionary = {}
  const event = event_innerEventCreationSteps(eventInterface, realm,
    new Date(), dictionary)
 ...
 * 
 */
export function event_innerEventCreationSteps(eventInterface: typeof EventImpl, realm: any,
 ...
 */
export function event_dispatch(event: Event, target: EventTarget,
  ...
  */
  ...
  let relatedTarget = tree_retarget(event._relatedTarget, target) as EventTarget
  ...
  if (target !== relatedTarget || target === event._relatedTarget) {
    ...
    let slotable =
      (Guard.isSlotable(target) && shadowTree_isAssigned(target)) ?
        target : null
    ...
    while (parent !== null && Guard.isNode(parent)) {
       ...
       */
      if (Guard.isSlotable(parent) && shadowTree_isAssigned(parent)) {
        slotable = parent
      }
      ...
      if (Guard.isWindow(parent) || (Guard.isNode(parent) && Guard.isNode(target) &&
        tree_isAncestorOf(tree_rootNode(target, true), parent, true, true))) {
        /**
        ...
        event_appendToAnEventPath(event, parent, null,
        ...
        event_appendToAnEventPath(event, parent, target,
          relatedTarget, touchTargets, slotInClosedTree)
      }
    ...
    if (clearTargetsStruct !== null) {
      if (Guard.isNode(clearTargetsStruct.shadowAdjustedTarget) &&
        Guard.isShadowRoot(tree_rootNode(clearTargetsStruct.shadowAdjustedTarget, true))) {
        clearTargets = true
 ...
 */
export function event_appendToAnEventPath(event: Event,
  ...
  if (Guard.isNode(invocationTarget) &&
    Guard.isShadowRoot(tree_rootNode(invocationTarget))) {
 ...
 * whether the event listener's callback threw an exception
 */
export function event_invoke(struct: EventPathItem, event: Event,
   ...
   */
  const found = event_innerInvoke(event, listeners, phase, struct,
    legacyOutputDidListenersThrowFlag)
  ...
  if (!found && event.isTrusted) {
     ...
     */
    event_innerInvoke(event, listeners, phase, struct,
      legacyOutputDidListenersThrowFlag)
 ...
 */
export function event_createLegacyEvent(eventInterface: string): Event {
  /**
 ...
 */
export function event_getterEventHandlerIDLAttribute(thisObj: EventTarget,
   ...
   */
  const eventTarget = event_determineTheTargetOfAnEventHandler(
    thisObj, name)
  if (eventTarget === null) return null
  return event_getTheCurrentValueOfAnEventHandler(
    eventTarget, name)
}
 ...
 */
export function event_setterEventHandlerIDLAttribute(thisObj: EventTarget,
 ...
 * @param name - event name
 */
export function event_determineTheTargetOfAnEventHandler(eventTarget: EventTarget,
 ...
 * @param name - event name
 */
export function event_getTheCurrentValueOfAnEventHandler(eventTarget: EventTarget,
  name: string): EventHandler {
  // TODO: Implement in HTML DOM
  return null
}
 ...
 * @param name - event name
 */
export function event_activateAnEventHandler(eventTarget: EventTarget,
  name: string): void {
  // TODO: Implement in HTML DOM
}
 ...
 * @param name - event name
 */
export function event_deactivateAnEventHandler(eventTarget: EventTarget,
  name: string): void {
  // TODO: Implement in HTML DOM
}
